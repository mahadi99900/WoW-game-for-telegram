...
import sys # এই লাইনটি একদম উপরে import os এর নিচে বসান

# --- নতুন কোড: টোকেন লোড করার জন্য ---
try:
    with open('config.json', 'r') as f:
        config = json.load(f)
    TELEGRAM_TOKEN = config['TELEGRAM_TOKEN']
    if not TELEGRAM_TOKEN:
        raise ValueError("Telegram token is empty in config.json")
except FileNotFoundError:
    print("Error: config.json not found! Please run 'bash setup.sh' first.")
    sys.exit(1)
except (ValueError, KeyError):
    print("Error: TELEGRAM_TOKEN not found or is empty in config.json. Please run 'bash setup.sh' again.")
    sys.exit(1)
OWNER_ID = 6776334925
